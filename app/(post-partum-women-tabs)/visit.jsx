...
export default function Visit() {
  ...
  const { visits, fetchVisits, loading } = useMockVisits();
  ...
  return (
    <SafeAreaView className="flex-1" edges={["top"]}>
      {/* Header */}
      <View
        className={`flex flex-row justify-between items-center px-4 ${topMargin} bg-[#FBE9E2] pt-4 pb-4`}
      >
        <TouchableOpacity
          className="w-[50px] h-[50px] bg-white flex flex-row items-center justify-center rounded-full"
          onPress={() => router.push("/(post-partum-women-tabs)")}
        >
