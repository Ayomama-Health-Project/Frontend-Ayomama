...
const PartnerLogIn = () => {
  ...
  const familyFriendSubtitleText = useTranslation(
    "Log in to support the mother as family or a trusted friend",
  );
  const partnerSubtitleText = useTranslation(
    "Log in to track your partner's journey",
  );
  const subtitleText =
    ...
    try {
      ...
      if (account?.role !== "partner") {
        ...
        Toast.show({
          ...
          text2: getRoleMismatchMessage("partner", account),
          position: "top",
        });
        setTimeout(() => {
          router.replace(
            getRoleLoginRoute(
              account?.role,
              account?.motherType || account?.healthWorkerType,
            ),
          );
        }, 600);
        return;
            ...
            <TouchableOpacity
              ...
              onPress={() =>
                router.push({
                  ...
                  params: { role: "partner", type, email },
                })
              }
            >
            ...
            <View className="py-8 items-center">
              <View className="flex-row items-center">
                ...
                <TouchableOpacity
                  onPress={() =>
                    router.push({
                      pathname: "/(auth)/auth/partner/signup",
                      params: { type },
                    })
                  }
                >
