  ...
  return (
    <View className="mb-4 rounded-[26px] border border-[#00D2B3] bg-white px-3 py-3">
      <View className="mb-2 flex-row items-start justify-between">
        ...
        <TouchableOpacity onPress={() => onDelete(item.id)} activeOpacity={0.8}>
      ...
      <View className="flex-row items-center">
        ...
        <View className="ml-3 flex-1">
          ...
          <View className="mt-3 flex-row flex-wrap justify-between">
            <MacroBar color="#1CC9B7" label="40g Protein" />
            <MacroBar color="#FF8A57" label="15g Fat" />
            <MacroBar color="#F4E38C" label="45g Carbs" />
          </View>
        </View>
      </View>
    </View>
...
export default function EditNutritionPlanForm({
  visible,
  meals,
  onClose,
  onDeleteMeal,
  ...
  return (
    <FormShell visible={visible} onClose={onClose} title="Edit Nutrition plan">
      <View className="rounded-[28px] border border-[#00D2B3] px-2 py-4">
            ...
            {["Breakfast", "Lunch", "Dinner"].map((tab) => (
              ...
              >
                <View className="py-2 items-center">
                  <Text className="text-[15px] font-bold data-[state=active]:text-white">{tab}</Text>
          ...
          <TabsTabPanels>
            {Object.entries(grouped).map(([category, items]) => (
              <TabsTabPanel key={category} value={category} className="px-0">
                {items.map((item) => (
                  <MealCard key={item.id} item={item} onDelete={onDeleteMeal} />
                ))}
              </TabsTabPanel>
            ))}
          </TabsTabPanels>
